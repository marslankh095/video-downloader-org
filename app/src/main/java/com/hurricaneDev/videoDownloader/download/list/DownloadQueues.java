...
package com.hurricaneDev.videoDownloader.download.list;
...
public class DownloadQueues implements Serializable {
    ...
    private String getValidName(String name, String type) {
        ...
        int i = 0;
        File file = new File(Environment.getExternalStoragePublicDirectory(VDApp.getInstance().getApplicationContext().getString(R.string.app_name)), name + "." + type);
        StringBuilder nameBuilder = new StringBuilder(name);
        while (file.exists()) {
            i++;
            nameBuilder = new StringBuilder(name);
            nameBuilder.append(" ").append(i);
            file = new File(Environment.getExternalStoragePublicDirectory(VDApp.getInstance().getApplicationContext().getString(R.string.app_name)), nameBuilder + "." + type);
        }
